...
void CTcpClient::initClient() noexcept(false)
{
    ...
    {
        throw CSocketException("CTcpClient::initClient : client{%s:%u} socket failed -> %s", IMainSocket::mIp.c_str(), IMainSocket::mPort, strerror(errno));
    }
	...
	{
		throw CSocketException("CTcpClient::initClient : client{%s:%u} connect failed -> %s", IMainSocket::mIp.c_str(), IMainSocket::mPort, strerror(errno));
	}
}
